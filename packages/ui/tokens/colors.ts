export const baseColors = {
  white: 'white',
  failure: '#f89292',
  failure33: '#ED4B9E33',
  primary0f: '#0088cc0f',
  primary3D: '#0088cc3D',
  primaryBright: '#53DEE9',
  primaryDark: '#05557e',
  success: '#39a9e1',
  success19: '#31D0AA19',
  warning: '#cbaa78',
  warning2D: '#ED4B9E2D',
  warning33: '#ED4B9E33',
}

export const additionalColors = {
  binance: '#F0B90B',
  overlay: '#452a7a',
  gold: '#FFC700',
  silver: '#B2B2B2',
  bronze: '#E7974D',
}

export const lightColors = {
  ...baseColors,
  ...additionalColors,
  primary: '#6ca9df',
  secondary: '#0088cc',
  secondary80: '#b99aff',
  background: '#c2c1c1',
  backgroundDisabled: '#ededed',
  backgroundAlt: '#f1f2f8',
  backgroundAlt2: '#dfdfdfb3',
  backgroundAlt3: '#f1f5ffb3',
  buttonBackground: 'linear-gradient(145deg, #e6e6e6, #ffffff)',
  buttonAlt: '#d9d9d9',
  buttonAlt1: '#ffffff',
  cardBorder: '#E4E8EE',
  chartEffects: '#e8e8e8',
  contrast: '#252527',
  dropdown: '#dcdcdc',
  dropdownDeep: '#EEEEEE',
  invertedContrast: '#FFFFFF',
  input: '#e7e7e7',
  inputSecondary: '#ececec',
  tertiary: '#e7e7e7',
  text: '#7b7b7b',
  text99: '#7b7b7b99',
  textDisabled: '#767676',
  textSubtle: '#919a9f',
  textSlide: '#85868b',
  disabled: '#E9EAEB',
  settings: '#b1c4d8',
  shadow: '#232323af',
  slideOut: '#e5e4ec',
  slideShadow: 'rgba(0,0,0,0.2)',
  slideBackground1: '#f2f2f2',
  gradientBubblegum: 'linear-gradient(139.73deg, #f9f9f9 0%, #d1d0f0 100%)',
  gradientInverseBubblegum: 'linear-gradient(139.73deg, #F3EFFF 0%, #E5FDFF 100%)',
  gradientCardHeader: 'linear-gradient(111.68deg, #F2ECF2 0%, #E8F2F6 100%)',
  gradientBlue: 'linear-gradient(180deg, #A7E8F1 0%, #94E1F2 100%)',
  gradientViolet: 'linear-gradient(180deg, #E2C9FB 0%, #CDB8FA 100%)',
  gradientVioletAlt: 'linear-gradient(180deg, #CBD7EF 0%, #9A9FD0 100%)',
  gradientGold: 'linear-gradient(180deg, #FFD800 0%, #FDAB32 100%)',
  gradientBold: 'linear-gradient(#53DEE9, #0088cc)',
  buttonBackground1: '#f2f2f2',
  buttonBackgroundHover: '#c4c4c4',
  buttonShadow: 'rgba(0,0,0,0.25)',
  menuShadow: 'rgba(0, 0, 0, 0.1)',
  menuShadowAlt: 'rgba(0, 0, 0, 0.15)',

}

export const darkColors = {
  ...baseColors,
  ...additionalColors,
  primary: '#6ca9df',
  secondary: '#08c',
  secondary80: '#b99aff',
  background: 'linear-gradient(90deg,#3b3a40,#353535)',
  backgroundDisabled: '#4d4d5c',
  backgroundAlt: '#2f2f37',
  backgroundAlt2: '#27262cb3',
  backgroundAlt3: '#3b3e40',
  buttonBackground: 'linear-gradient(145deg, #515558, #36393b)',
  buttonAlt: '#1c1c1c',
  buttonAlt1: '#282c34',
  cardBorder: '#3c3c3f',
  chartEffects: '#FFFFFF',
  contrast: '#FFFFFF',
  dropdown: '#1E1D20',
  dropdownDeep: '#212529',
  invertedContrast: '#2b2b2b',
  input: '#312e36',
  inputSecondary: '#353535',
  primaryDark: '#05557e',
  tertiary: '#30303a',
  text: '#F8F8F2',
  text99: '#F4EEFF99',
  textDisabled: '#a9a9a9',
  textSubtle: '#ffffff',
  textSlide: '#d6d6d6',
  disabled: '#524B63',
  settings: '#ffffff',
  shadow: '#ffffff6f',
  slideOut: '#2f3137',
  slideShadow: 'rgba(0, 0, 0, 0.5)',
  slideBackground1: '#14141496',
  gradientBubblegum: 'linear-gradient(139.73deg, #5f5f71 0%, #1e1e25 100%)',
  gradientInverseBubblegum: 'linear-gradient(139.73deg, #1E1D20 0%, #353547 100%)',
  gradientCardHeader: 'linear-gradient(166.77deg, #3B4155 0%, #3A3045 100%)',
  gradientBlue: 'linear-gradient(180deg, #00707F 0%, #19778C 100%)',
  gradientViolet: 'linear-gradient(180deg, #6C4999 0%, #6D4DB2 100%)',
  gradientVioletAlt: 'linear-gradient(180deg, #434575 0%, #66578D 100%)',
  gradientGold: 'linear-gradient(180deg, #FFD800 0%, #FDAB32 100%)',
  gradientBold: 'linear-gradient(#53DEE9, #9A6AFF)',
  buttonBackground1: '#4d4d4d',
  buttonBackgroundHover: '#1c1c1c',
  buttonShadow: 'rgba(212, 212, 212, 0.15)',
  menuShadow: ' rgba(0, 0, 0, 0.382)', 
  menuShadowAlt: 'rgba(0, 0, 0, 0.7)',
}
